...
public class HtcAjaxShowOneDeckRenderer extends HtmlRenderer
{
    ...
    UIComponent  component) throws IOException
  {
    ...
    String showItemURL = handler.getResourceURL(context, "weblet://com.apress.projsf.ch9/showItem.htc");

    ResponseWriter out = context.getResponseWriter();
    writeScriptInline(context, "document.namespaces.add('pro', 'http://projsf.apress.com/tags');");
    out.write("<?import namespace=\"pro\" implementation=\"" + showItemURL + "\" ?>");
    ...
    UIComponent  component) throws IOException
  {
    if (component.getChildCount() > 0)
    {
      ...
      String styleClass = (String)attrs.get(ITEM_STYLE_CLASS_ATTR);
      String headerStyleClass = (String)attrs.get(ITEM_HEADER_STYLE_CLASS_ATTR);
      String contentStyleClass = (String)attrs.get(ITEM_CONTENT_STYLE_CLASS_ATTR);
      ...
      for (Iterator iter = children.iterator(); iter.hasNext();)
      {
        ...
        if (child instanceof UIShowItem)
        {
          ...
          // the header facet
          UIComponent header = showItem.getFacet("header");
          if (header != null)
          {
            out.startElement("pro:headerFacet", null);
